...
if (window.localStorage.getItem('secretLang') === null) {
  if (langSub === 'zh') {
    localStorage.setItem('secretLang', 'CN')
    lan = 'CN'
  } else {
    localStorage.setItem('secretLang', 'EN')
    lan = 'EN'
  }
  i18n.locale = lan
} else { // 根据用户上次的选择的语言来显示语言
  i18n.locale = window.localStorage.getItem('secretLang')
}
